...
//		log_message('error', json_encode(array_slice($args, 1)));

		if (get_class ( self::$Db ) == "PDO")
			return call_user_func_array ( "self::sqlPdo", $args );
        else if (get_class ( self::$Db ) == "CI_DB_mysqli_driver")
			return call_user_func_array ( "self::sqlCIDB", $args );
		else
